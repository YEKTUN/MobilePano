...
function EmailPropertiesPanel({ selectedPanoId, selectedComponent }) {
  ...
  if (!selectedComponent || selectedComponent.type !== "Email") {
    return (
      <div>
        <h2 className="font-bold mb-2">Properties</h2>
        <p>No **Email** component selected.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 border rounded-md bg-white shadow-md w-[600px]">
      ...
      <select
        name="inputType"
        value={formData.inputType || "Email"}
        onChange={handleChange}
        className="border rounded px-2 py-1 w-full mb-3"
      >
      ...
      <div className="flex justify-between items-center mb-3">
        <label className="flex font-medium">
          Allow Multiple Email IDs?
          <div className="w-5 h-5 ml-2 border-2 rounded-full flex justify-center items-center">?</div>
        </label>
        <Switch
          checked={!!formData.allowMultipleEmails}
          onCheckedChange={(checked) => handleSwitchChange("allowMultipleEmails", checked)}
        />
      </div>
