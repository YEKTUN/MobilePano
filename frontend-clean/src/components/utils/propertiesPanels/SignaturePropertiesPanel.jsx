...
function SignaturePropertiesPanel({ selectedPanoId, selectedComponent }) {
  ...
  if (!selectedComponent || selectedComponent.type !== "Signature") {
    return (
      <div>
        <h2 className="font-bold mb-2">Properties</h2>
        <p>No **Signature** component selected.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 border rounded-md bg-white shadow-md w-[600px]">
      ...
      <h2 className="text-lg font-bold mb-4">Signature Properties</h2>
      ...
      <label className="flex font-medium mb-1">Enter Note <div className="w-5 h-5 ml-2 border-2 rounded-full flex justify-center items-center">?</div></label>
      <input
        type="text"
        name="note"
        value={formData.note || ""}
