...
function WebLinkPropertiesPanel({ selectedPanoId, selectedComponent }) {
  ...
  if (!selectedComponent || selectedComponent.type !== "WebLink") {
    return (
      <div>
        <h2 className="font-bold mb-2">Properties</h2>
        <p>No **WebLink** component selected.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 border rounded-md bg-white shadow-md w-[600px] ">
      ...
      <h2 className="text-lg font-bold mb-4">Web Link Properties</h2>
      ...
      <input
        type="text"
        name="targetURL"
        value={formData.targetURL || "http://www.google.com"}
        onChange={handleChange}
        className="border rounded px-2 py-1 w-full mb-3"
      />
...
export default WebLinkPropertiesPanel;
