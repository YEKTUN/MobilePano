...
function SubformPropertiesPanel({ selectedPanoId, selectedComponent }) {
  ...
  if (!selectedComponent || selectedComponent.type !== "Subform") {
    return (
      <div>
        <h2 className="font-bold mb-2">Properties</h2>
        <p>No **Subform** component selected.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 border rounded-md bg-white shadow-md w-[600px]">
      ...
      <h2 className="text-lg font-bold mb-4">Subform Properties</h2>
      ...
      <select
        name="entryMode"
        value={formData.entryMode || "Default"}
        onChange={handleChange}
        className="border rounded px-2 py-1 w-full mb-3"
      >
...
export default SubformPropertiesPanel;
